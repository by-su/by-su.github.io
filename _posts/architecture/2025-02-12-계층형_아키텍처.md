...
1. 애플리케이션의 기능을 구분 짓는 패키지 경계가 없다. 예를 들어서 teacher 도메인이 추가되면 다음과 같이 서로 연관되지 않은 기능들이 모이게 된다.
...
2. 애플리케이션이 어떤 유스케이스를 제공하는지 파악할 수 없다. TeacherService가 어떤 유스케이스를 구현했는지 알 수 없다. 
3. 패키지 구조를 통해 목표로 하는 아키텍처를 파악할 수 없다.


### 기능으로 패키지 구성하기
위에 제시된 문제는 기능 또는 특징 별로 구성하면 해결될 것 같기도 하다. 
```
myschool/
├── student/
│   ├── Student
|   ├── StudentRepository
|   ├── StudentService
|   ├── StudnetRepositoryImpl
|   ├── StudentController
```
하지만 해당 방법도 계층형 패키지 구조가 가진 2,3번 문제를 해결하지 못한다. 또한, package-private 접근 수준을 이용하고 있기 때문에 도메인 코드가 실수로 영속성 코드에 의존하는 것을 막을 수 없다.(StudnetService가 StudentRepositoryImpl을 사용할 수 있음.)
